...
try { db.run("ALTER TABLE gastos_extraordinarios ADD COLUMN aporte_caja REAL DEFAULT 0"); } catch(_) {}
try { db.run("ALTER TABLE gastos_pagos ADD COLUMN comprobante_path TEXT"); } catch(_) {}

db.run(`CREATE TABLE IF NOT EXISTS gastos_pagos (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  comprobante_path TEXT,
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (plantilla_id) REFERENCES gastos_plantilla(id)
)`);
...
app.get("/api/pagos/resumen", (c) => {
  ...
  // Abonos a gastos fijos (pagos parciales registrados este mes)
  const pagosRes = db.query("SELECT COALESCE(SUM(monto),0) as t FROM gastos_pagos WHERE mes=? AND anio=?").get(mes, anio) as any;
  const total_pagos_fijos = pagosRes?.t ?? 0;
  ...
  // Ahorro / faltante del mes (abonos cuentan como egreso real)
  const total_egresos = total_gastos + total_misc + total_pagos_fijos;
  ...
  return c.json({ pagaron, pendientes, recaudado, cuota_total, porcentaje,
    total_condominios: condos.length, total_gastos, total_misc, total_pagos_fijos, gastos_fijos,
    ahorro_mes, cobertura_fijos, saldo_caja, saldo_caja_bruto, saldo_inicial, deposito_extra,
    aportes_comprometidos, total_ext, recaudado_ext });
});
...
// ════════════════════════════════════════════
//  LIBRO DE CAJA — movimientos entrada/salida
// ════════════════════════════════════════════
app.get("/api/movimientos", (c) => {
  const mes  = c.req.query("mes")  ? Number(c.req.query("mes"))  : null;
  const anio = Number(c.req.query("anio") ?? new Date().getFullYear());

  const mesWhere = mes ? "AND mes=?" : "";
  const mesArgs  = mes ? [anio, mes] : [anio];
  const mesArgs2 = mes ? [mes, anio] : [null, anio]; // para tablas con otro orden

  // ── Ingresos: cuotas ordinarias ──────────────────────────────
  const pagosQ = mes
    ? "SELECT p.id, p.fecha_pago as fecha, p.monto, p.mes, p.anio, p.metodo_pago, p.referencia, p.notas, c.unidad, c.propietario, 'ingreso' as tipo, 'cuota' as categoria, 'pagos' as tabla FROM pagos p JOIN condominios c ON c.id=p.condominio_id WHERE p.anio=? AND p.mes=? ORDER BY p.fecha_pago, p.created_at"
    : "SELECT p.id, p.fecha_pago as fecha, p.monto, p.mes, p.anio, p.metodo_pago, p.referencia, p.notas, c.unidad, c.propietario, 'ingreso' as tipo, 'cuota' as categoria, 'pagos' as tabla FROM pagos p JOIN condominios c ON c.id=p.condominio_id WHERE p.anio=? ORDER BY p.fecha_pago, p.created_at";
  const pagos = mes
    ? db.query(pagosQ).all(anio, mes) as any[]
    : db.query(pagosQ).all(anio) as any[];

  // ── Egresos: gastos ordinarios ────────────────────────────────
  const gastosQ = mes
    ? "SELECT id, fecha, monto, mes, anio, NULL as metodo_pago, NULL as referencia, notas, NULL as unidad, proveedor as propietario, 'egreso' as tipo, categoria, 'gastos' as tabla FROM gastos WHERE anio=? AND mes=? ORDER BY fecha, created_at"
    : "SELECT id, fecha, monto, mes, anio, NULL as metodo_pago, NULL as referencia, notas, NULL as unidad, proveedor as propietario, 'egreso' as tipo, categoria, 'gastos' as tabla FROM gastos WHERE anio=? ORDER BY fecha, created_at";
  const gastos = mes
    ? db.query(gastosQ).all(anio, mes) as any[]
    : db.query(gastosQ).all(anio) as any[];

  // ── Egresos: misceláneos ──────────────────────────────────────
  const miscQ = mes
    ? "SELECT id, fecha, monto, mes, anio, NULL as metodo_pago, NULL as referencia, notas, NULL as unidad, 'Misceláneo' as propietario, 'egreso' as tipo, 'misc' as categoria, 'gastos_misc' as tabla FROM gastos_misc WHERE anio=? AND mes=? ORDER BY fecha, created_at"
    : "SELECT id, fecha, monto, mes, anio, NULL as metodo_pago, NULL as referencia, notas, NULL as unidad, 'Misceláneo' as propietario, 'egreso' as tipo, 'misc' as categoria, 'gastos_misc' as tabla FROM gastos_misc WHERE anio=? ORDER BY fecha, created_at";
  const misc = mes
    ? db.query(miscQ).all(anio, mes) as any[]
    : db.query(miscQ).all(anio) as any[];

  // ── Egresos: abonos a proveedores fijos ───────────────────────
  const abonosQ = mes
    ? "SELECT gp.id, gp.fecha, gp.monto, gp.mes, gp.anio, NULL as metodo_pago, NULL as referencia, gp.notas, NULL as unidad, pl.proveedor as propietario, 'egreso' as tipo, pl.categoria, 'gastos_pagos' as tabla FROM gastos_pagos gp JOIN gastos_plantilla pl ON pl.id=gp.plantilla_id WHERE gp.anio=? AND gp.mes=? ORDER BY gp.fecha, gp.created_at"
    : "SELECT gp.id, gp.fecha, gp.monto, gp.mes, gp.anio, NULL as metodo_pago, NULL as referencia, gp.notas, NULL as unidad, pl.proveedor as propietario, 'egreso' as tipo, pl.categoria, 'gastos_pagos' as tabla FROM gastos_pagos gp JOIN gastos_plantilla pl ON pl.id=gp.plantilla_id WHERE gp.anio=? ORDER BY gp.fecha, gp.created_at";
  const abonos = mes
    ? db.query(abonosQ).all(anio, mes) as any[]
    : db.query(abonosQ).all(anio) as any[];

  // Unir y ordenar por fecha, luego mes
  const todos = [...pagos, ...gastos, ...misc, ...abonos].sort((a: any, b: any) => {
    const fa = a.fecha || `${a.anio}-${String(a.mes).padStart(2,'0')}-01`;
    const fb = b.fecha || `${b.anio}-${String(b.mes).padStart(2,'0')}-01`;
    return fa < fb ? -1 : fa > fb ? 1 : 0;
  });

  // Calcular saldo acumulado
  let saldo = 0;
  const movimientos = (todos as any[]).map((m: any) => {
    saldo += m.tipo === 'ingreso' ? m.monto : -m.monto;
    return { ...m, saldo_acumulado: Math.round(saldo * 100) / 100 };
  });

  const total_ingresos = (todos as any[]).filter((m: any) => m.tipo === 'ingreso').reduce((s: number, m: any) => s + m.monto, 0);
  const total_egresos  = (todos as any[]).filter((m: any) => m.tipo === 'egreso').reduce((s: number, m: any) => s + m.monto, 0);

  return c.json({ movimientos, total_ingresos, total_egresos, saldo_final: total_ingresos - total_egresos });
});
...
app.post("/api/gastos", requireAdmin, async (c) => {
  ...
  db.run(
    ...
    [b.concepto, b.categoria, Number(b.monto), Number(b.mes), Number(b.anio),
     b.fecha || new Date().toISOString().slice(0,10), b.proveedor || null, b.notas || null, b.es_recurrente ? 1 : 0]
  );
  return c.json({ ok: true });
});
...
// ── Pagos parciales a gastos fijos ──
app.get("/api/gastos/plantilla/:id/pagos", (c) => {
  const id = Number(c.req.param("id"));
  const mes  = Number(c.req.query("mes")  ?? new Date().getMonth() + 1);
  const anio = Number(c.req.query("anio") ?? new Date().getFullYear());
  const plantilla = db.query("SELECT * FROM gastos_plantilla WHERE id=?").get(id) as any;
  if (!plantilla) return c.json({ error: "No encontrado" }, 404);
  const pagos = db.query(
    "SELECT * FROM gastos_pagos WHERE plantilla_id=? AND mes=? AND anio=? ORDER BY created_at ASC"
  ).all(id, mes, anio) as any[];
  const pagado = pagos.reduce((s: number, p: any) => s + p.monto, 0);
  const pendiente = Math.max(0, plantilla.monto - pagado);
  return c.json({ plantilla, pagos, pagado, pendiente, mes, anio });
});

app.post("/api/gastos/plantilla/:id/pagos", requireAdmin, async (c) => {
  const id = Number(c.req.param("id"));
  const b = await c.req.parseBody();
  const mes   = Number(b.mes)   || (new Date().getMonth() + 1);
  const anio  = Number(b.anio)  || new Date().getFullYear();
  const monto = Number(b.monto);
  if (!monto || monto <= 0) return c.json({ error: "Monto inválido" }, 400);
  const plantilla = db.query("SELECT * FROM gastos_plantilla WHERE id=?").get(id) as any;
  const pagado = (db.query(
    "SELECT SUM(monto) as t FROM gastos_pagos WHERE plantilla_id=? AND mes=? AND anio=?"
  ).get(id, mes, anio) as any)?.t ?? 0;
  if (pagado + monto > plantilla.monto * 1.001) {
    return c.json({ error: `El pago excede el monto pendiente ($${(plantilla.monto - pagado).toFixed(2)})` }, 400);
  }
  let comprobante_path: string | null = null;
  if (b.comprobante && typeof b.comprobante !== "string") {
    comprobante_path = await saveFile(b.comprobante as File, "comprobantes");
  }
  db.run(
    "INSERT INTO gastos_pagos (plantilla_id,mes,anio,monto,fecha,notas,comprobante_path) VALUES (?,?,?,?,?,?,?)",
    [id, mes, anio, monto, b.fecha || new Date().toISOString().slice(0,10), b.notas || null, comprobante_path]
  );
  return c.json({ ok: true });
});

app.delete("/api/gastos/pagos/:id", requireAdmin, (c) => {
  const row = db.query("SELECT comprobante_path FROM gastos_pagos WHERE id=?").get(Number(c.req.param("id"))) as any;
  if (row?.comprobante_path) {
    try { require("fs").unlinkSync(row.comprobante_path.replace("/uploads/", `${UPLOAD_DIR}/`)); } catch {}
  }
  db.run("DELETE FROM gastos_pagos WHERE id=?", [Number(c.req.param("id"))]);
  return c.json({ ok: true });
});
...
app.get("/api/gastos-ext", (c) => {
  ...
  return c.json(gex.map(g => {
    // Monto neto a repartir = total - aporte de caja chica
    const aporte_caja   = g.aporte_caja ?? 0;
    const monto_neto    = Math.max(0, g.monto_total - aporte_caja);
    const cuotas_por_condo = condos.map((co: any) => ({
      ...co,
      cuota_ext: totalCuotas > 0 ? Math.round((co.cuota_mensual / totalCuotas) * monto_neto * 100) / 100 : 0,
    }));
    ...
    return { ...g, aporte_caja, monto_neto, cuotas_por_condo, recaudado, presupuestos };
  }));
});
...
app.post("/api/gastos-ext", requireAdmin, async (c) => {
  const b = await c.req.json();
  const r = db.run(
    "INSERT INTO gastos_extraordinarios (titulo,descripcion,monto_total,aporte_caja,num_cuotas,mes_inicio,anio_inicio) VALUES (?,?,?,?,?,?,?)",
    [b.titulo, b.descripcion || "", Number(b.monto_total), Number(b.aporte_caja) || 0,
     Number(b.num_cuotas) || 1, Number(b.mes_inicio), Number(b.anio_inicio)]
  );
...
app.put("/api/gastos-ext/:id", requireAdmin, async (c) => {
  const b = await c.req.json();
  db.run(
    "UPDATE gastos_extraordinarios SET titulo=?,descripcion=?,monto_total=?,aporte_caja=?,num_cuotas=?,estado=? WHERE id=?",
    [b.titulo, b.descripcion || "", Number(b.monto_total), Number(b.aporte_caja) || 0,
     Number(b.num_cuotas), b.estado || "activo", Number(c.req.param("id"))]
  );
  return c.json({ ok: true });
});
...
app.put("/api/actas/:id", requireAdmin, async (c) => {
  const b = await c.req.json();
  db.run(
    "UPDATE actas SET fecha=?,temas=?,asistentes=?,num_asistentes=?,total_condominos=?,resoluciones=? WHERE id=?",
    [b.fecha, b.temas, b.asistentes||"", Number(b.num_asistentes)||0,
     Number(b.total_condominos)||16, b.resoluciones||"", Number(c.req.param("id"))]
  );
  return c.json({ ok: true });
});
...
app.delete("/api/actas/:id", requireAdmin, (c) => {
  db.run("DELETE FROM actas WHERE id=?", [Number(c.req.param("id"))]);
  return c.json({ ok: true });
});
...
:root{
  ...
  --text:#1a1710;--text2:#3d3520;--text3:#6b6045;
  --r:16px;--rs:10px
}
...
#canvas-firma,#canvas-firma-presup{cursor:crosshair;width:100%;touch-action:none;display:block;border-radius:inherit}
/* Situaciones */
...
@media(max-width:900px){
  ...
  /* Tipografía más grande en móvil para adultos mayores */
  body{font-size:15px}
  .nav-item{font-size:15px;padding:11px 14px}
  .stat-lbl{font-size:12px}
  .stat-val{font-size:26px}
  .stat-sub{font-size:13px}
  .ctitle{font-size:13px}
  .log-entry{padding:14px 0}
  .card{padding:18px}
  .fi,.fs,.fta{font-size:15px;padding:11px 14px}
...
<aside class='sidebar' id='sb'>
  ...
  <nav class='nav'>
    <div class='nav-sec' id='nav-sec-finanzas'>
      ...
      <button class='nav-item' id='nav-caja' onclick='go("caja",this)'>📒 Libro de Caja</button>
    </div>
...
<main class='main'>
  ...
  <div class='content'>
    ...
    <div class='page' id='page-caja'>
      <div class='sec-hdr'>
        <div style='display:flex;gap:10px;align-items:center;flex-wrap:wrap'>
          <select class='fs' id='caja-mes' style='width:110px' onchange='loadCaja()'>
          ...
          <select class='fs' id='caja-anio' style='width:90px' onchange='loadCaja()'>
            <option value='2026' selected>2026</option>
            <option value='2027'>2027</option>
          </select>
        </div>
      </div>
      <div class='g3' style='margin-bottom:18px' id='caja-stats'></div>
      <div class='card' style='padding:0;overflow:hidden'>
        <div class='tw' id='caja-tabla'></div>
      </div>
    </div>
    ...
    <div class='page' id='page-gastos-ext'>
      <div class='sec-hdr'>
        <div></div>
        <div id='btn-nuevo-gext' style='display:none'><button class='btn btn-g' onclick='abrirNuevoGext()'>+ Nuevo gasto</button></div>
      </div>
    ...
    <div class='page' id='page-actas'>
      <div class='sec-hdr'>
        ...
        <div id='btn-acta' style='display:none'><button class='btn btn-g' onclick='abrirNuevaActa()'>📝 Nueva acta</button></div>
      </div>
...
<!-- Modal pagos parciales gastos fijos -->
<div class='modal-ov' id='modal-pagos-parciales' onclick='closeOv(event,"modal-pagos-parciales")'><div class='modal mw'>
  <div class='modal-title'>💸 Abonos — <span id='pp-titulo'></span></div>
  <div style='background:var(--bg3);border:1px solid var(--border);border-radius:12px;padding:14px;margin-bottom:14px'>
    <div style='display:flex;justify-content:space-between;font-size:12px;color:var(--text3);margin-bottom:8px'>
      <span>Total del gasto: <strong id='pp-total' style='color:var(--text)'></strong></span>
    </div>
    <div id='pp-resumen'></div>
  </div>
  <div class='ctitle' style='margin-bottom:8px'>Historial de abonos este mes</div>
  <div id='pp-historial' style='max-height:200px;overflow-y:auto;margin-bottom:14px'></div>
  <div style='background:var(--bg3);border:1px solid var(--border);border-radius:12px;padding:14px'>
    <div class='ctitle' style='margin-bottom:10px'>Registrar abono</div>
    <div class='g2'>
      <div class='form-g'><label class='form-l'>Monto ($)</label><input class='fi' id='pp-monto' type='number' min='0' step='0.01' placeholder='0.00'></div>
      <div class='form-g'><label class='form-l'>Fecha</label><input class='fi' id='pp-fecha' type='date'></div>
    </div>
    <div class='form-g'><label class='form-l'>Notas (opcional)</label><input class='fi' id='pp-notas' placeholder='Ej: Primer quincena'></div>
    <div class='form-g'>
      <label class='form-l'>Comprobante de pago</label>
      <label style='display:flex;align-items:center;gap:10px;cursor:pointer'>
        <input type='file' id='pp-comprobante' accept='image/*' style='display:none' onchange='ppFotoReady(this)'>
        <span class='btn btn-o btn-sm' onclick='$id("pp-comprobante").click()'>📷 Foto / archivo</span>
        <span id='pp-foto-status' style='font-size:12px;color:var(--text3)'>Sin adjunto</span>
      </label>
    </div>
    <button class='btn btn-g' style='margin-top:10px;width:100%' onclick='guardarPagoParcial()'>✓ Registrar abono</button>
  </div>
  <button class='btn btn-o' style='margin-top:10px;width:100%' onclick='closeModal("modal-pagos-parciales")'>Cerrar</button>
</div></div>
<!-- Plantilla gastos -->
...
<div class='modal-ov' id='modal-nuevo-gext' onclick='closeOv(event,"modal-nuevo-gext")'><div class='modal mw'>
  <input type='hidden' id='gext-edit-id'>
  <div class='modal-title' id='gext-modal-titulo'>💎 Gasto Extraordinario</div>
  ...
  <div class='g2'>
    <div class='form-g'><label class='form-l'>Monto total ($)</label><input class='fi' id='gext-monto' type='number' oninput='recalcularGext()'></div>
  ...
  <div style='background:var(--bg3);border:1px solid var(--border);border-radius:14px;padding:16px;margin:8px 0'>
    <div style='font-size:12px;font-weight:700;color:var(--text3);letter-spacing:1px;text-transform:uppercase;margin-bottom:10px'>🏦 Aporte desde Caja Chica</div>
    <div class='form-g' style='margin-bottom:8px'>
      <label class='form-l'>Monto a aportar de caja chica ($)</label>
      <input class='fi' id='gext-aporte-caja' type='number' min='0' value='0' oninput='recalcularGext()' placeholder='0'>
      <div style='font-size:11px;color:var(--text3);margin-top:4px'>Este monto se descuenta del total antes de repartir entre condóminos</div>
    </div>
    <div id='gext-preview' style='display:none;background:var(--bg2);border-radius:10px;padding:12px;font-size:13px'>
      <div style='display:flex;justify-content:space-between;margin-bottom:5px'>
        <span style='color:var(--text2)'>Monto total:</span>
        <span id='gext-prev-total' style='font-family:"DM Mono",monospace;font-weight:700'></span>
      </div>
      <div style='display:flex;justify-content:space-between;margin-bottom:5px'>
        <span style='color:var(--blue)'>− Aporte caja chica:</span>
        <span id='gext-prev-caja' style='font-family:"DM Mono",monospace;color:var(--blue)'></span>
      </div>
      <div style='display:flex;justify-content:space-between;padding-top:8px;border-top:1px solid var(--border)'>
        <span style='font-weight:700'>A repartir entre condóminos:</span>
        <span id='gext-prev-neto' style='font-family:"DM Mono",monospace;font-weight:700;color:var(--gold2)'></span>
      </div>
    </div>
  </div>
  <div style='display:flex;gap:9px'><button class='btn btn-g' id='gext-btn-guardar' onclick='guardarGastoExt()'>Crear</button><button class='btn btn-o' onclick='closeModal("modal-nuevo-gext")'>Cancelar</button></div>
</div></div>
...
<div class='modal-ov' id='modal-presupuesto' onclick='closeOv(event,"modal-presupuesto")'><div class='modal mw'>
  ...
  <div class='form-g'><label class='form-l'>Firma digital</label>
    <div style='background:#fff;border-radius:var(--rs);border:2px solid var(--border2);overflow:hidden;margin-bottom:8px'><canvas id='canvas-firma-presup' height='180'></canvas></div>
...
<div class='modal-ov' id='modal-votar' onclick='closeOv(event,"modal-votar")'><div class='modal mw'>
  ...
  <div class='form-g'><label class='form-l'>Firma digital</label>
    <div style='background:#fff;border-radius:var(--rs);border:2px solid var(--border2);overflow:hidden;margin-bottom:8px'><canvas id='canvas-firma' height='180'></canvas></div>
...
<div class='modal-ov' id='modal-acta' onclick='closeOv(event,"modal-acta")'><div class='modal mw'>
  <input type='hidden' id='acta-edit-id'>
  <div class='modal-title' id='acta-modal-titulo'>📖 Nueva Acta</div>
  <div class='g2'>
  ...
  <div style='display:flex;gap:9px'><button class='btn btn-g' id='acta-btn-guardar' onclick='guardarActa()'>Guardar</button><button class='btn btn-o' onclick='closeModal("modal-acta")'>Cancelar</button></div>
</div></div>
