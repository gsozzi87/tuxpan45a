...
app.post("/api/gastos/plantilla/:id/pagos", requireAdmin, async (c) => {
  ...
  // Verificar que no exceda el monto del gasto
  const plantilla = db.query("SELECT * FROM gastos_plantilla WHERE id=?").get(id) as any;
  ...
  db.run(
    "INSERT INTO gastos_pagos (plantilla_id,mes,anio,monto,fecha,notas) VALUES (?,?,?,?,?,?)",
    [id, mes, anio, monto, b.fecha || new Date().toISOString().slice(0,10), b.notas || null]
  );
  return c.json({ ok: true });
});
...
<div class='modal-ov' id='modal-pagos-parciales' onclick='closeOv(event,"modal-pagos-parciales")'><div class='modal mw'>
  ...
  <div id='pp-historial' style='max-height:180px;overflow-y:auto;margin-bottom:14px'></div>
  <div style='background:var(--bg3);border:1px solid var(--border);border-radius:12px;padding:14px'>
    ...
    <button class='btn btn-g' style='margin-top:8px;width:100%' onclick='guardarPagoParcial()'>✓ Registrar abono</button>
  </div>
