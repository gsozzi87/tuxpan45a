...
async function loadDash() {
  ...
  $id('dash-prog').innerHTML = `
    ...
    <div style="display:flex;flex-direction:column;gap:7px;padding-top:10px;border-top:1px solid var(--border)">
      ...
      ${total_gastos > 0 && total_gastos !== gastos_fijos ? `
      <div style="display:flex;justify-content:space-between;font-size:13px">
        <span style="color:var(--text2)">Total egresos registrados</span>
  ...
  $id('pp-historial').innerHTML = info.pagos.length
    ? info.pagos.map(p => `
        <div style="display:flex;align-items:center;gap:8px;padding:7px 0;border-bottom:1px solid var(--border)">
          ...
          <div style="font-family:'DM Mono',monospace;font-size:13px;color:var(--green)">${fmtM(p.monto)}</div>
...
async function guardarPagoParcial() {
  const mes = Number($id('fil-mes-g').value) || (new Date().getMonth() + 1);
  const r = await api(`/api/gastos/plantilla/${_ppId}/pagos`, {
    method: 'POST',
    body: JSON.stringify({
      monto: $id('pp-monto').value,
      fecha: $id('pp-fecha').value,
      notas: $id('pp-notas').value,
      mes, anio: 2026,
    }),
  });
  if (r.ok) {
