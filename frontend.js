...
function go(p, b) {
  ...
  ({
    ...
    gastos: () => { loadGastos(); loadGastosMisc(); }, 'gastos-ext': loadGastosExt, caja: loadCaja,
...
function aplicarSesion() {
  ...
  const adminEls = ['btn-reg-pago','btn-nuevo-gasto','btn-acta','btn-nueva-vot',
                    ...
                    'gasto-th-del','btn-condos-side','btn-nuevo-aviso'];
  ...
  const hideFromManto = [
    ...
    'nav-votaciones', 'nav-situaciones', 'nav-mantenimiento', 'nav-caja',
...
async function loadDash() {
  ...
  const [r, av, deudores] = await Promise.all([
    ...
    api('/api/avisos'),
    api(`/api/pagos/deudores?anio=${anio}`),
  ]);

  const MESES_NOM = ['','Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic'];
  const mesNom    = MESES_NOM[mes] || mes;

  // ── Cálculos clave ──
  const cuota_total    = r.cuota_total   || 0;
  const recaudado      = r.recaudado     || 0;
  const gastos_fijos   = r.gastos_fijos  || 0;
  const total_gastos   = (r.total_gastos || 0) + (r.total_misc || 0);
  const ahorro_mes     = r.ahorro_mes    || 0;
  const saldo_caja     = r.saldo_caja    ?? 0;
  const porcentaje     = r.porcentaje    || 0;

  // Adeudo mes actual: condos que no pagaron este mes
  const deudores_mes   = deudores.filter(d => d.meses_sin_pagar.includes(mes));
  const adeudo_mes     = deudores_mes.reduce((s, d) => s + d.cuota_mensual, 0);

  // Adeudo total anual (todos los meses sin pagar)
  const adeudo_anio    = deudores.reduce((s, d) => s + d.adeudo, 0);
  const n_deudores     = deudores.length;

  // Falta para cubrir gastos fijos
  const falta_fijos    = Math.max(0, gastos_fijos - recaudado);
  const pct_fijos      = gastos_fijos > 0 ? Math.min(100, Math.round((recaudado / gastos_fijos) * 100)) : 100;
  const fijos_color    = pct_fijos >= 100 ? 'var(--green)' : pct_fijos >= 70 ? 'var(--gold)' : 'var(--red)';

  const ahorroColor  = ahorro_mes >= 0 ? 'var(--green)' : 'var(--red)';
  const cajaColor    = saldo_caja >= 0 ? 'var(--blue)'  : 'var(--red)';

  // ══ FILA 1: 4 stats clave ══
  $id('dash-stats').innerHTML = `
    <div class="stat" style="--ac:var(--green)">
      <div class="stat-lbl">Pagaron ${mesNom}</div>
      <div class="stat-val">${r.pagaron ?? 0}</div>
      <div class="stat-sub">de ${r.total_condominios} unidades activas · <strong style="color:${r.pendientes > 0 ? 'var(--red)' : 'var(--green)'}">${r.pendientes} pendientes</strong></div>
    </div>
    <div class="stat" style="--ac:var(--gold)">
      <div class="stat-lbl">Recaudado ${mesNom}</div>
      <div class="stat-val" style="font-size:19px">${fmtM(recaudado)}</div>
      <div class="stat-sub">de ${fmtM(cuota_total)} esperado · <strong style="color:${porcentaje>=100?'var(--green)':'var(--gold)'}">${porcentaje}%</strong></div>
    </div>
    <div class="stat" style="--ac:${ahorroColor}">
      <div class="stat-lbl">${ahorro_mes >= 0 ? 'Ahorro del mes' : 'Déficit del mes'}</div>
      <div class="stat-val" style="font-size:19px;color:${ahorroColor}">${fmtM(Math.abs(ahorro_mes))}</div>
      <div class="stat-sub">Gastos registrados: ${fmtM(total_gastos)}</div>
    </div>
    <div class="stat" style="--ac:${cajaColor}">
      <div class="stat-lbl">Caja chica</div>
      <div class="stat-val" style="font-size:19px;color:${cajaColor}">${fmtM(saldo_caja)}</div>
      <div class="stat-sub">${r.aportes_comprometidos > 0
        ? `Bruto ${fmtM(r.saldo_caja_bruto)} · <span style="color:var(--blue)">−${fmtM(r.aportes_comprometidos)} gtos. ext.</span>`
        : 'Saldo disponible'}</div>
    </div>`;

  // ══ FILA 2: adeudos del mes + adeudo anual ══
  $id('dash-adeudos').innerHTML = `
    <div class="stat" style="--ac:var(--red)">
      <div class="stat-lbl">⚠️ Adeudo este mes</div>
      <div class="stat-val" style="font-size:19px;color:${adeudo_mes > 0 ? 'var(--red)' : 'var(--green)'}">${adeudo_mes > 0 ? fmtM(adeudo_mes) : '—'}</div>
      <div class="stat-sub">
        ${adeudo_mes > 0
          ? `${deudores_mes.length} unidad${deudores_mes.length !== 1 ? 'es' : ''} sin pagar ${mesNom}: ${deudores_mes.map(d => d.unidad).join(', ')}`
          : `✅ Todas las unidades pagaron ${mesNom}`}
      </div>
    </div>
    <div class="stat" style="--ac:var(--purple)">
      <div class="stat-lbl">📋 Adeudo acumulado ${anio}</div>
      <div class="stat-val" style="font-size:19px;color:${adeudo_anio > 0 ? 'var(--purple)' : 'var(--green)'}">${adeudo_anio > 0 ? fmtM(adeudo_anio) : '—'}</div>
      <div class="stat-sub">
        ${adeudo_anio > 0
          ? `${n_deudores} deudor${n_deudores !== 1 ? 'es' : ''} · <a href="#" onclick="go('deudores',null);return false" style="color:var(--blue)">Ver detalle →</a>`
          : '✅ Sin adeudos acumulados'}
      </div>
    </div>`;

  // ══ FILA 3a: Recaudación vs gastos fijos ══
  // Egresos reales = solo lo efectivamente pagado
  const abonos_fijos   = r.total_pagos_fijos || 0;
  const gastos_otros   = (r.total_gastos || 0) + (r.total_misc || 0);
  const egresos_reales = abonos_fijos + gastos_otros;
  const balance_real   = recaudado - egresos_reales;
  const balanceColor   = balance_real >= 0 ? 'var(--green)' : 'var(--red)';

  // Pendiente de pagar a proveedores fijos este mes
  const fijos_pendiente = Math.max(0, gastos_fijos - abonos_fijos);
  const fijos_pct       = gastos_fijos > 0 ? Math.min(100, Math.round((abonos_fijos / gastos_fijos) * 100)) : 100;
  const fijos_color2    = fijos_pct >= 100 ? 'var(--green)' : fijos_pct > 0 ? 'var(--gold)' : 'var(--red)';

  // Cobertura: ¿alcanza lo recaudado para cubrir los fijos?
  const cubre_fijos = recaudado >= gastos_fijos;

  $id('dash-prog').innerHTML = `
    <div style="display:flex;flex-direction:column;gap:0">

      <!-- Bloque A: Ingresos vs compromiso -->
      <div style="margin-bottom:14px">
        <div style="font-size:11px;font-weight:700;color:var(--text3);letter-spacing:.8px;text-transform:uppercase;margin-bottom:8px">Cuotas del mes</div>
        <div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:4px">
          <span style="color:var(--text2)">Recaudado</span>
          <span style="font-family:'DM Mono',monospace;font-weight:700;color:var(--green)">${fmtM(recaudado)}</span>
        </div>
        <div style="display:flex;justify-content:space-between;font-size:12px;color:var(--text3)">
          <span>de ${fmtM(cuota_total)} esperado · ${porcentaje}%</span>
          <span style="color:${recaudado < cuota_total ? 'var(--red)' : 'var(--green)'}">${recaudado < cuota_total ? '⚠️ Falta: '+fmtM(cuota_total - recaudado) : '✅ Completo'}</span>
        </div>
      </div>

      <!-- Bloque B: Pagos a proveedores fijos -->
      <div style="margin-bottom:14px;padding-top:12px;border-top:1px solid var(--border)">
        <div style="font-size:11px;font-weight:700;color:var(--text3);letter-spacing:.8px;text-transform:uppercase;margin-bottom:8px">Pagos a proveedores</div>
        <div class="prog" style="margin-bottom:5px">
          <div class="prog-b" style="width:${fijos_pct}%;background:${fijos_color2}"></div>
        </div>
        <div style="display:flex;justify-content:space-between;font-size:12px;color:var(--text3);margin-bottom:8px">
          <span>${fijos_pct}% pagado de ${fmtM(gastos_fijos)}</span>
          <span style="color:${fijos_pendiente > 0 ? 'var(--gold)' : 'var(--green)'}">${fijos_pendiente > 0 ? '⏳ Pendiente: '+fmtM(fijos_pendiente) : '✅ Saldado'}</span>
        </div>
        <div style="display:flex;justify-content:space-between;font-size:13px">
          <span style="color:var(--text2)">Abonado hasta hoy</span>
          <span style="font-family:'DM Mono',monospace;color:var(--red)">−${fmtM(abonos_fijos)}</span>
        </div>
        ${gastos_otros > 0 ? `
        <div style="display:flex;justify-content:space-between;font-size:13px;margin-top:5px">
          <span style="color:var(--text2)">Otros gastos</span>
          <span style="font-family:'DM Mono',monospace;color:var(--red)">−${fmtM(gastos_otros)}</span>
        </div>` : ''}
      </div>

      <!-- Bloque C: Balance real -->
      <div style="padding-top:12px;border-top:2px solid var(--border)">
        <div style="display:flex;justify-content:space-between;font-size:14px;font-weight:700">
          <span>${balance_real >= 0 ? 'Disponible en caja' : 'Déficit'}</span>
          <span style="font-family:'DM Mono',monospace;color:${balanceColor}">${fmtM(Math.abs(balance_real))}</span>
        </div>
        <div style="font-size:11px;color:var(--text3);margin-top:4px">
          ${fmtM(recaudado)} recaudado − ${fmtM(egresos_reales)} egresado
          ${fijos_pendiente > 0 ? ` · <span style="color:var(--gold)">aún queda ${fmtM(fijos_pendiente)} por pagar a proveedores</span>` : ''}
        </div>
      </div>
  ...
  const tiLabel = { info:'Info', urgente:'🚨 Urgente', comunicado:'Comunicado', mantenimiento:'🔧 Mantenimiento' };
  $id('dash-avisos').innerHTML = av.length
    ? av.slice(0, 4).map(a => `
    <div class="log-entry" style="align-items:flex-start">
      <div class="ldot ${dcol[a.tipo] || ''}" style="margin-top:4px;flex-shrink:0"></div>
      <div style="flex:1;min-width:0">
        <div style="font-size:11px;color:var(--text3);font-family:'DM Mono',monospace;margin-bottom:2px">${a.created_at?.slice(0,10) || ''} · ${tiLabel[a.tipo] || a.tipo}</div>
        <div style="font-size:14px;font-weight:700;line-height:1.3;margin-bottom:4px">${a.titulo}</div>
        ${a.contenido ? `<div style="font-size:13px;color:var(--text2);line-height:1.55">${a.contenido.slice(0,120)}${a.contenido.length>120?'…':''}</div>` : ''}
      </div>
    </div>`).join('')
    + `<div style="text-align:center;margin-top:12px"><a href="#" onclick="go('avisos',null);return false" style="font-size:13px;color:var(--blue);font-weight:600">Ver todos los avisos →</a></div>`
    : '<div style="color:var(--text3);font-size:13px;padding:20px 0;text-align:center">Sin avisos publicados</div>';
}
...
async function loadCuentas() {
  ...
  const rows = condos.map(co => {
    const celdas = co.meses.map(({ mes, anio, pago }) => {
      ...
      if (pago) return `<td style="text-align:center">
        <span class="mes-dot pagado" title="${fmtM(pago.monto)} · ${pago.numero_recibo || ''}">✓</span>
        ${sesion?.rol === 'admin' ? `<br><button class="btn btn-r" style="font-size:10px;padding:2px 6px;margin-top:2px;line-height:1" onclick="borrarPago(${pago.id},'${co.propietario}','${MESES[mes]}')">✕</button>` : ''}
      </td>`;
...
async function borrarPago(id, propietario, mes) {
  if (!confirm(`¿Eliminar el pago de ${propietario} de ${mes}?\nEsta acción no se puede deshacer.`)) return;
  const r = await api(`/api/pagos/${id}`, { method: 'DELETE' });
  if (r.ok) { loadCuentas(); loadDash(); loadDeudores(); showToast('🗑️ Pago eliminado'); }
  else showToast('Error al eliminar', false);
}

// ── LIBRO DE CAJA ────────────────────────
const CATICONS2 = { seguridad:'🔒', elevador:'🛗', servicios:'💡', mantenimiento:'🔧', misc:'📦', otro:'📦' };

async function loadCaja() {
  const mes  = $id('caja-mes')?.value  || '';
  const anio = $id('caja-anio')?.value || '2026';
  const data = await api(`/api/movimientos?anio=${anio}${mes ? '&mes='+mes : ''}`);

  // ── Stats ──
  const balColor = data.saldo_final >= 0 ? 'var(--green)' : 'var(--red)';
  $id('caja-stats').innerHTML = `
    <div class="stat" style="--ac:var(--green)">
      <div class="stat-lbl">Total ingresos</div>
      <div class="stat-val" style="font-size:20px;color:var(--green)">${fmtM(data.total_ingresos)}</div>
      <div class="stat-sub">${data.movimientos.filter(m=>m.tipo==='ingreso').length} movimientos</div>
    </div>
    <div class="stat" style="--ac:var(--red)">
      <div class="stat-lbl">Total egresos</div>
      <div class="stat-val" style="font-size:20px;color:var(--red)">${fmtM(data.total_egresos)}</div>
      <div class="stat-sub">${data.movimientos.filter(m=>m.tipo==='egreso').length} movimientos</div>
    </div>
    <div class="stat" style="--ac:${balColor}">
      <div class="stat-lbl">Saldo del período</div>
      <div class="stat-val" style="font-size:20px;color:${balColor}">${fmtM(Math.abs(data.saldo_final))}</div>
      <div class="stat-sub">${data.saldo_final >= 0 ? 'Positivo ✅' : '⚠️ Negativo'}</div>
    </div>`;

  // ── Tabla ──
  if (!data.movimientos.length) {
    $id('caja-tabla').innerHTML = '<div style="text-align:center;padding:60px;color:var(--text3)"><div style="font-size:48px;margin-bottom:14px">📒</div><div>Sin movimientos en este período</div></div>';
    return;
  }

  const isAdmin = sesion?.rol === 'admin';
  const catLabel = { cuota:'Cuota condómino', seguridad:'Seguridad', elevador:'Elevador', servicios:'Servicios', mantenimiento:'Mantenimiento', misc:'Misceláneo', otro:'Otro' };

  $id('caja-tabla').innerHTML = `
    <table style="width:100%;border-collapse:collapse;font-size:13px">
      <thead>
        <tr style="background:var(--bg3);border-bottom:2px solid var(--border)">
          <th style="padding:10px 14px;text-align:left;font-size:11px;letter-spacing:.8px;text-transform:uppercase;color:var(--text3)">Fecha</th>
          <th style="padding:10px 14px;text-align:left;font-size:11px;letter-spacing:.8px;text-transform:uppercase;color:var(--text3)">Concepto</th>
          <th style="padding:10px 14px;text-align:right;font-size:11px;letter-spacing:.8px;text-transform:uppercase;color:var(--green)">Ingreso</th>
          <th style="padding:10px 14px;text-align:right;font-size:11px;letter-spacing:.8px;text-transform:uppercase;color:var(--red)">Egreso</th>
          <th style="padding:10px 14px;text-align:right;font-size:11px;letter-spacing:.8px;text-transform:uppercase;color:var(--text3)">Saldo</th>
      ...
      <tbody>
        ${data.movimientos.map((m, i) => {
          ...
          const fecha     = m.fecha ? m.fecha.slice(0,10) : `${m.anio}-${String(m.mes).padStart(2,'0')}-01`;
          const concepto  = esIngreso
            ? `<strong>${m.unidad}</strong> — ${m.propietario}`
            : `${CATICONS2[m.categoria]||'📦'} ${catLabel[m.categoria]||m.categoria}${m.propietario ? ' · '+m.propietario : ''}`;
          const nota      = m.notas ? `<div style="font-size:11px;color:var(--text3);margin-top:2px">${m.notas}</div>` : '';
          const saldoColor = m.saldo_acumulado >= 0 ? 'var(--text)' : 'var(--red)';
          const rowBg = i % 2 === 0 ? '' : 'background:var(--bg2)';
          const delBtn = isAdmin
            ? `<td style="padding:8px 14px;text-align:center"><button class="btn btn-r btn-sm" style="font-size:11px;padding:3px 7px" onclick="borrarMovimiento('${m.tabla}',${m.id},'${concepto.replace(/'/g,"\'").replace(/<[^>]+>/g,'')}')">✕</button></td>`
            : '';
          return `
            <tr style="border-bottom:1px solid var(--border);${rowBg}">
              <td style="padding:10px 14px;white-space:nowrap;color:var(--text3);font-size:12px;font-family:'DM Mono',monospace">${fecha}</td>
              <td style="padding:10px 14px">${concepto}${nota}</td>
              <td style="padding:10px 14px;text-align:right;font-family:'DM Mono',monospace;font-weight:700;color:var(--green)">${esIngreso ? fmtM(m.monto) : ''}</td>
              <td style="padding:10px 14px;text-align:right;font-family:'DM Mono',monospace;font-weight:700;color:var(--red)">${!esIngreso ? fmtM(m.monto) : ''}</td>
              <td style="padding:10px 14px;text-align:right;font-family:'DM Mono',monospace;font-weight:600;color:${saldoColor}">${fmtM(m.saldo_acumulado)}</td>
              ${delBtn}
            </tr>`;
        }).join('')}
      </tbody>
...
async function borrarMovimiento(tabla, id, concepto) {
  if (!confirm(`¿Eliminar el movimiento:\n"${concepto}"?\n\nEsta acción actualizará el saldo y no se puede deshacer.`)) return;
  const endpoints = {
    pagos:        `/api/pagos/${id}`,
    gastos:       `/api/gastos/${id}`,
    gastos_misc:  `/api/gastos-misc/${id}`,
    gastos_pagos: `/api/gastos/pagos/${id}`,
  };
  const url = endpoints[tabla];
  if (!url) { showToast('Tipo de movimiento no reconocido', false); return; }
  const r = await api(url, { method: 'DELETE' });
  if (r.ok) {
    loadCaja();
    loadDash();
    if (tabla === 'pagos') { loadCuentas(); loadDeudores(); }
...
async function loadGastos() {
  ...
  // Cargar pagos del mes para cada gasto fijo
  const mesFil = Number($id('fil-mes-g').value) || (new Date().getMonth() + 1);
  const pagosPlantilla = await Promise.all(
    plantilla.map(g => api(`/api/gastos/plantilla/${g.id}/pagos?mes=${mesFil}&anio=2026`))
  );
  $id('plantilla-list').innerHTML = plantilla.length
    ? plantilla.map((g, i) => {
        const info    = pagosPlantilla[i];
        const pagado  = info?.pagado ?? 0;
        const pct     = g.monto > 0 ? Math.min(100, Math.round((pagado / g.monto) * 100)) : 0;
        const color   = pct >= 100 ? 'var(--green)' : pct > 0 ? 'var(--gold)' : 'var(--red)';
        const badge   = pct >= 100 ? '✅ Pagado' : pct > 0 ? `⏳ ${pct}%` : '⚠️ Pendiente';
        return `
        <div class="log-entry" style="flex-direction:column;align-items:stretch;gap:6px;padding:10px 0">
          <div style="display:flex;align-items:center;gap:8px">
            ...
            <div style="text-align:right">
              ...
              <div style="font-size:10px;color:${color};font-weight:700">${badge}</div>
            </div>
            ${sesion?.rol==='admin' ? `<button class="btn btn-o btn-sm" style="white-space:nowrap" onclick="abrirPagosParciales(${g.id},'${(g.concepto||'').replace(/'/g,"\\'")}',${g.monto})">💸 Abonar</button>` : ''}
          </div>
          ${pagado > 0 ? `
          <div style="padding-left:24px">
            <div style="display:flex;justify-content:space-between;font-size:11px;color:var(--text3);margin-bottom:3px">
              <span>Abonado: ${fmtM(pagado)}</span><span>Pendiente: ${fmtM(Math.max(0,g.monto-pagado))}</span>
            </div>
            <div class="prog"><div class="prog-b" style="width:${pct}%;background:${color}"></div></div>
          </div>` : ''}
        </div>`}).join('')
...
// ── Pagos parciales de gastos fijos ──
let _ppId = null, _ppConcepto = '', _ppMonto = 0;

async function abrirPagosParciales(id, concepto, monto) {
  _ppId = id; _ppConcepto = concepto; _ppMonto = monto;
  $id('pp-titulo').textContent  = concepto;
  $id('pp-total').textContent   = fmtM(monto);
  $id('pp-monto').value         = '';
  $id('pp-fecha').value         = new Date().toISOString().slice(0,10);
  $id('pp-notas').value         = '';
  const fi = $id('pp-comprobante');
  if (fi) fi.value = '';
  const lbl = $id('pp-comp-label');
  if (lbl) lbl.textContent = '📎 Adjuntar comprobante';
  await refreshPagosParciales();
  openModal('modal-pagos-parciales');
}

async function refreshPagosParciales() {
  const mes = Number($id('fil-mes-g').value) || (new Date().getMonth() + 1);
  const info = await api(`/api/gastos/plantilla/${_ppId}/pagos?mes=${mes}&anio=2026`);
  const pct  = _ppMonto > 0 ? Math.min(100, Math.round((info.pagado / _ppMonto) * 100)) : 0;
  const color = pct >= 100 ? 'var(--green)' : pct > 0 ? 'var(--gold)' : 'var(--red)';

  $id('pp-resumen').innerHTML = `
    <div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:6px">
      <span style="color:var(--text2)">Pagado:</span>
      <span style="font-family:'DM Mono',monospace;font-weight:700;color:${color}">${fmtM(info.pagado)}</span>
    </div>
    <div class="prog" style="margin-bottom:6px"><div class="prog-b" style="width:${pct}%;background:${color}"></div></div>
    <div style="display:flex;justify-content:space-between;font-size:12px;color:var(--text3)">
      <span>${pct}% completado</span>
      <span>Pendiente: <strong style="color:var(--red)">${fmtM(info.pendiente)}</strong></span>
    </div>`;

  $id('pp-historial').innerHTML = info.pagos.length
    ? info.pagos.map(p => `
        <div style="display:flex;align-items:center;gap:8px;padding:8px 0;border-bottom:1px solid var(--border)">
          <div style="flex:1;min-width:0">
            <div style="font-size:12px;font-weight:600">${p.fecha}</div>
            ${p.notas ? `<div style="font-size:11px;color:var(--text3)">${p.notas}</div>` : ''}
            ${p.comprobante_path ? `<a href="${p.comprobante_path}" target="_blank" style="font-size:11px;color:var(--blue);text-decoration:none">📎 Ver comprobante</a>` : ''}
          </div>
          <div style="font-family:'DM Mono',monospace;font-size:13px;color:var(--green);white-space:nowrap">${fmtM(p.monto)}</div>
          <button class="btn btn-r btn-sm" onclick="delPagoParcial(${p.id})">✕</button>
        </div>`).join('')
    : '<div style="color:var(--text3);font-size:12px;padding:14px 0;text-align:center">Sin abonos registrados este mes</div>';

  // Autocompletar con pendiente
  if (!$id('pp-monto').value) $id('pp-monto').value = info.pendiente > 0 ? info.pendiente.toFixed(2) : '';
}

async function guardarPagoParcial() {
  const mes = Number($id('fil-mes-g').value) || (new Date().getMonth() + 1);
  const fd  = new FormData();
  fd.append('monto', $id('pp-monto').value);
  fd.append('fecha', $id('pp-fecha').value);
  fd.append('notas', $id('pp-notas').value);
  fd.append('mes',   mes);
  fd.append('anio',  '2026');
  const fileInput = $id('pp-comprobante');
  if (fileInput?.files?.[0]) fd.append('comprobante', fileInput.files[0]);
  const r = await apiF(`/api/gastos/plantilla/${_ppId}/pagos`, fd);
  if (r.ok) {
    ...
    $id('pp-monto').value = '';
    $id('pp-notas').value = '';
    if (fileInput) { fileInput.value = ''; $id('pp-comp-label').textContent = '📎 Adjuntar comprobante'; }
  } else if (r.error) {
    showToast('⚠️ ' + r.error);
  }
}

async function delPagoParcial(id) {
  if (!confirm('¿Eliminar este abono?')) return;
  await api(`/api/gastos/pagos/${id}`, { method: 'DELETE' });
  await refreshPagosParciales();
  loadGastos();
  loadDash();
...
async function registrarPago() {
  ...
  if (r.ok) { closeModal('modal-pago'); loadCuentas(); loadDash(); showToast('✓ Pago registrado'); }
  else showToast('Error: ' + r.error, false);
}

// ── GASTOS EXTRAORDINARIOS ────────────────
let _gextCache = [];
async function editarGextById(id) {
  // Si el caché está vacío, refrescar
  if (!_gextCache.length) _gextCache = await api('/api/gastos-ext');
  const g = _gextCache.find(x => x.id == id);
  if (g) abrirEditGext(g);
  else showToast('Error: recargue la página');
}
  ...
  if (!gex.length) {
    $id('gext-list').innerHTML = `
      <div style="text-align:center;padding:60px;color:var(--text3)">
        ...
        ${sesion?.rol==='admin' ? `<button class="btn btn-g" style="margin-top:16px" onclick="abrirNuevoGext()">+ Crear primero</button>` : ''}
      </div>`;
    return;
  }
      ...
      <div class="gext-card">
        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:14px;margin-bottom:14px">
          ...
          <div style="text-align:right;flex-shrink:0">
            ...
            <div style="font-size:11px;color:var(--text3);margin-top:3px">Monto total</div>
            ${g.aporte_caja > 0 ? `
              <div style="font-size:11px;color:var(--blue);margin-top:2px">🏦 Caja chica: −${fmtM(g.aporte_caja)}</div>
              <div style="font-size:12px;font-weight:700;color:var(--gold2);margin-top:2px">Neto a repartir: ${fmtM(g.monto_neto)}</div>
            ` : ''}
          </div>
        </div>
        <div style="margin-bottom:14px">
        ...
        <div style="margin-bottom:14px">
          <div class="ctitle">Cuota por despacho${g.aporte_caja > 0 ? ` <span style="font-size:11px;color:var(--blue);font-weight:400">(sobre ${fmtM(g.monto_neto)} neto)</span>` : ""}</div>
...
function recalcularGext() {
  const total  = parseFloat($id('gext-monto').value) || 0;
  const caja   = parseFloat($id('gext-aporte-caja').value) || 0;
  const neto   = Math.max(0, total - caja);
  const prev   = $id('gext-preview');
  if (total > 0 && caja > 0) {
    $id('gext-prev-total').textContent = fmtM(total);
    $id('gext-prev-caja').textContent  = fmtM(caja);
    $id('gext-prev-neto').textContent  = fmtM(neto);
    prev.style.display = '';
  } else {
...
function abrirNuevoGext() {
  $id('gext-edit-id').value     = '';
  $id('gext-modal-titulo').textContent = '💎 Nuevo Gasto Extraordinario';
  $id('gext-titulo').value      = '';
  $id('gext-desc').value        = '';
  $id('gext-monto').value       = '';
  $id('gext-cuotas').value      = '1';
  $id('gext-aporte-caja').value = '0';
  $id('gext-preview').style.display = 'none';
  $id('gext-btn-guardar').textContent = 'Crear';
  openModal('modal-nuevo-gext');
}

function abrirEditGext(g) {
  $id('gext-edit-id').value     = g.id;
  $id('gext-modal-titulo').textContent = '✏️ Editar Gasto Extraordinario';
  $id('gext-titulo').value      = g.titulo || '';
  $id('gext-desc').value        = g.descripcion || '';
  $id('gext-monto').value       = g.monto_total || '';
  $id('gext-cuotas').value      = g.num_cuotas || 1;
  $id('gext-aporte-caja').value = g.aporte_caja || 0;
  $id('gext-btn-guardar').textContent = '💾 Guardar cambios';
  recalcularGext();
  openModal('modal-nuevo-gext');
}

async function guardarGastoExt() {
  const editId = $id('gext-edit-id').value;
  const body = {
    titulo:       $id('gext-titulo').value,
    descripcion:  $id('gext-desc').value,
    monto_total:  $id('gext-monto').value,
    aporte_caja:  $id('gext-aporte-caja').value || 0,
    num_cuotas:   $id('gext-cuotas').value,
    mes_inicio:   $id('gext-mes-ini').value,
    anio_inicio:  $id('gext-anio-ini').value,
  };
  const url    = editId ? `/api/gastos-ext/${editId}` : '/api/gastos-ext';
...
async function loadActas() {
  _actasCache = await api('/api/actas');
  if (!_actasCache.length) {
  ...
  $id('actas-list').innerHTML = _actasCache.map(a => `
    <div class="card" style="margin-bottom:14px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap;gap:9px">
        ...
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          ...
          ${sesion?.rol === 'admin' ? `
            ${a.estado !== 'firmada' ? `<button class="btn btn-g btn-sm" onclick="firmarActa(${a.id})">✍️ Firmar</button>` : ''}
            <button class="btn btn-o btn-sm" onclick="editarActa(${a.id})">✏️ Editar</button>
            <button class="btn btn-r btn-sm" onclick="borrarActa(${a.id})">🗑️ Borrar</button>
          ` : ''}
        </div>
      </div>
      <div style="margin-bottom:13px">
...
function abrirNuevaActa() {
  $id('acta-edit-id').value = '';
  $id('acta-modal-titulo').textContent = '📝 Nueva Acta';
  $id('acta-fecha').value     = new Date().toISOString().slice(0,10);
  $id('acta-temas').value     = '';
  $id('acta-asist').value     = '';
  $id('acta-num').value       = '';
  $id('acta-tot').value       = '16';
  $id('acta-res').value       = '';
  $id('acta-btn-guardar').textContent = 'Guardar';
  openModal('modal-acta');
}

function editarActa(id) {
  const a = _actasCache.find(x => x.id === id);
  if (!a) { showToast('Recargá la página'); return; }
  $id('acta-edit-id').value   = id;
  $id('acta-modal-titulo').textContent = `✏️ Editar Acta #${String(a.folio||0).padStart(3,'0')}`;
  $id('acta-fecha').value     = a.fecha || '';
  $id('acta-temas').value     = a.temas || '';
  $id('acta-asist').value     = a.asistentes || '';
  $id('acta-num').value       = a.num_asistentes || '';
  $id('acta-tot').value       = a.total_condominos || 16;
  $id('acta-res').value       = a.resoluciones || '';
  $id('acta-btn-guardar').textContent = '💾 Guardar cambios';
  openModal('modal-acta');
}

async function guardarActa() {
  const editId = $id('acta-edit-id').value;
  const body = {
  ...
  const url    = editId ? `/api/actas/${editId}` : '/api/actas';
  const method = editId ? 'PUT' : 'POST';
  const r = await api(url, { method, body: JSON.stringify(body) });
  if (r.ok) {
    closeModal('modal-acta');
    loadActas();
    showToast(editId ? '✓ Acta actualizada' : `✓ Acta #${r.folio} guardada`);
  }
}

async function borrarActa(id) {
  const a = _actasCache.find(x => x.id === id);
  const folio = a ? `#${String(a.folio||0).padStart(3,'0')}` : '';
  if (!confirm(`¿Eliminar el acta ${folio}? Esta acción no se puede deshacer.`)) return;
  const r = await api(`/api/actas/${id}`, { method: 'DELETE' });
  if (r.ok) { loadActas(); showToast(`🗑️ Acta ${folio} eliminada`); }
}

async function firmarActa(id) {
  if (!confirm('¿Firmar esta acta? Ya no podrá editarse.')) return;
...
// ── CANVAS: Motor de firma compartido ──────
function _setupCanvas(canvas, stateBridge) {
  // 1. Ajustar resolución al DPR del dispositivo (evita trazo borroso en Retina/móvil)
  const dpr = window.devicePixelRatio || 1;
  const rect = canvas.getBoundingClientRect();
  const cssW = rect.width  || canvas.offsetWidth  || 480;
  const cssH = rect.height || canvas.offsetHeight || 180;
  ...
  ctx.scale(dpr, dpr);               // todo lo que dibujemos usa coordenadas CSS
  ctx.fillStyle = '#fff';
  ...
  // Guardar ctx en el canvas para acceso externo
  canvas._ctx  = ctx;
  canvas._cssW = cssW;
  canvas._cssH = cssH;

  const bridge = stateBridge(ctx);

  // 2. Suavizado con curvas cuadráticas (punto medio entre puntos consecutivos)
  let pts = [];   // buffer de puntos para suavizado
  let drawing = false;
  ...
  function startStroke(pos) {
    ...
    pts = [pos, pos];   // duplicar para tener punto de control desde el inicio
    ctx.beginPath();
  ...
  function continueStroke(pos) {
    ...
    // Dibujar curva suavizada entre los últimos 3 puntos
  ...
  function endStroke() {
    ...
    // Cerrar el trazo con el último punto pendiente
    if (pts.length >= 2) {
  ...
  // Mouse
  canvas.onmousedown  = e => { e.preventDefault(); startStroke(getPos(e)); };
  canvas.onmousemove  = e => { continueStroke(getPos(e)); };
  canvas.onmouseup    = () => endStroke();
  canvas.onmouseleave = () => endStroke();

  // Touch — preventDefault evita scroll accidental mientras se firma
  canvas.ontouchstart = e => { e.preventDefault(); startStroke(getPos(e)); };
  canvas.ontouchmove  = e => { e.preventDefault(); continueStroke(getPos(e)); };
  canvas.ontouchend   = e => { e.preventDefault(); endStroke(); };
...
function initCanvas() {
  ...
  _setupCanvas(firmaCanvas, () => ({
    getDrawing: () => dibujandoFirma,
    setDrawing: v => { dibujandoFirma = v; }
  }));
  firmaCtx = firmaCanvas._ctx;
}
...
function initCanvasPresup() {
  ...
  _setupCanvas(firmaPresupCanvas, () => {
    return { getDrawing: () => dibujandoPresup, setDrawing: v => { dibujandoPresup = v; firmaPresupCtx = firmaPresupCanvas._ctx; } };
  });
  firmaPresupCtx = firmaPresupCanvas._ctx;
}
function limpiarFirmaPresup() {
  if (firmaPresupCtx && firmaPresupCanvas) {
    firmaPresupCtx.fillStyle = '#fff';
    firmaPresupCtx.fillRect(0, 0, firmaPresupCanvas.width, firmaPresupCanvas.height);
  }
}
