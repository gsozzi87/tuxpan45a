  ...
  $id('dash-avisos').innerHTML = av.length
    ...
    <div class="log-entry">
      ...
      <div style="flex:1;min-width:0">
        <div style="font-size:11px;color:var(--text3);font-family:'DM Mono',monospace">${a.created_at?.slice(0,10) || ''} · ${a.tipo || 'info'}</div>
        <div style="font-size:13px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${a.titulo}</div>
        ${a.contenido ? `<div style="font-size:11px;color:var(--text3);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${a.contenido.slice(0,80)}${a.contenido.length>80?'…':''}</div>` : ''}
      </div>
    </div>`).join('')
    + (av.length > 5 ? `<div style="text-align:center;margin-top:10px"><a href="#" onclick="go('avisos',null);return false" style="font-size:12px;color:var(--blue)">Ver todos (${av.length}) →</a></div>` : '')
