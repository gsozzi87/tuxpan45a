...
async function loadDash() {
  ...
  $id('dash-prog').innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:10px">
      <span style="font-size:13px;color:var(--text2)">Gastos fijos del mes</span>
      <span style="font-family:'DM Mono',monospace;font-size:14px;font-weight:700;color:var(--red)">${fmtM(gastos_fijos)}</span>
    </div>
    <div class="prog" style="margin-bottom:5px"><div class="prog-b" style="width:${pct_fijos}%;background:${fijos_color}"></div></div>
    <div style="display:flex;justify-content:space-between;font-size:12px;color:var(--text3);margin-bottom:14px">
      <span>${pct_fijos}% cubierto</span>
      <span style="color:${falta_fijos > 0 ? 'var(--red)' : 'var(--green);font-weight:700'}">${falta_fijos > 0 ? '⚠️ Falta: ' + fmtM(falta_fijos) : '✅ Cubierto'}</span>
    </div>
    <div style="display:flex;flex-direction:column;gap:7px;padding-top:10px;border-top:1px solid var(--border)">
      <div style="display:flex;justify-content:space-between;font-size:13px">
        <span style="color:var(--text2)">Cuota total esperada</span>
        <span style="font-family:'DM Mono',monospace;color:var(--gold2)">${fmtM(cuota_total)}</span>
      </div>
      <div style="display:flex;justify-content:space-between;font-size:13px">
        <span style="color:var(--text2)">Recaudado</span>
        <span style="font-family:'DM Mono',monospace;color:var(--green)">${fmtM(recaudado)}</span>
      </div>
      <div style="display:flex;justify-content:space-between;font-size:13px">
        <span style="color:var(--text2)">Gastos fijos</span>
        <span style="font-family:'DM Mono',monospace;color:var(--red)">−${fmtM(gastos_fijos)}</span>
      </div>
      ${(r.total_pagos_fijos||0) > 0 ? `
      <div style="display:flex;justify-content:space-between;font-size:13px">
        <span style="color:var(--text2)">Abonos a gastos fijos</span>
        <span style="font-family:'DM Mono',monospace;color:var(--red)">−${fmtM(r.total_pagos_fijos)}</span>
      </div>` : ''}
      ${total_gastos > 0 ? `
      <div style="display:flex;justify-content:space-between;font-size:13px">
        <span style="color:var(--text2)">Otros gastos registrados</span>
        <span style="font-family:'DM Mono',monospace;color:var(--red)">−${fmtM(total_gastos)}</span>
      </div>` : ''}
      <div style="display:flex;justify-content:space-between;font-size:13px;padding-top:7px;border-top:1px solid var(--border)">
        <span style="font-weight:700">${ahorro_mes >= 0 ? 'Balance' : 'Déficit'}</span>
        <span style="font-family:'DM Mono',monospace;font-weight:700;color:${ahorroColor}">${ahorro_mes >= 0 ? '' : '−'}${fmtM(Math.abs(ahorro_mes))}</span>
      </div>
    </div>`;
